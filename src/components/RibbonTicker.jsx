...
export default function RibbonTicker() {
    ...
    // "..." (spread) copies the array's items, so this lists the Blends twice
    // The css scrolls left by exactly half the width and snaps back, the rest is invisible and the loop looks endless
        ...
        <div className="ribbon-ticker-container">
            <div className="ribbon-ticker">
                ...
                <div className="ribbon-track">
                    {items.map((name, i) => (
                        <motion.span
                            // Index is a safe key here: the list never changes, and the items are unique
                            key={i}
                            className="ribbon-item"
                            // While hovered: grow to 110% and change amber
                            whileHover={{ scale: 1.1, color: "#f0b955" }}
                            // spring = bounce rather than linear higher stiffness is snappier
                            transition={{ type: "spring", stiffness: 300 }}>
